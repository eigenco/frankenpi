...
g++ -O3 -c opl.cpp
gcc -O0 pcserv5.c opl.o -o pcserv5 -lwiringPi -lpthread -lasound -lm
*/
...
int main(void) {
  int adlib_register, adlib_data, linear_address, c, h, s, rdd=0, run=1;
  unsigned char addr, data, ad, chs_state, rd=0;
  unsigned char *hdd;
  //unsigned char written[256*16*63];
  pthread_t thread;
  FILE *hdd_file;

  //for(c=0; c<256*16*63; c++)
  //  written[c] = 0;
  ...
  fwrite(hdd, 1, 132120576, hdd_file);
  fclose(hdd_file);
  free(hdd);
