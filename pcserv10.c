/*
gcc -O3 pcserv10.c opl.o -lwiringPi -lpthread -lm
*/
...
static int adlib_busy = 0;
static short buf[128];

void adlib(void) {
  for(;;)
    if(digitalRead(AREQ) && adlib_busy==0) {
...
int main(void) {
  ...
  unsigned char addr, data, *hdd, areg, abyte;
  FILE *hdd_file;
  pthread_t th;
  ...
  for(;;) {
    ...
    if(adlib_busy) {
      ...
      digitalWrite(ACLK, 1);
      digitalWrite(ACLK, 1);
      outd(buf[2*(64-adlib_busy)]&255);
      digitalWrite(ACLK, 0);
      digitalWrite(ACLK, 0);
      digitalWrite(ACLK, 0);

      outd(buf[2*(64-adlib_busy)+1]>>8);
      digitalWrite(ACLK, 1);
      digitalWrite(ACLK, 1);
      digitalWrite(ACLK, 1);
      outd(buf[2*(64-adlib_busy)+1]&255);
      digitalWrite(ACLK, 0);
      digitalWrite(ACLK, 0);
      digitalWrite(ACLK, 0);
