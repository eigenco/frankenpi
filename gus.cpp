...
typedef uintptr_t          Bitu; // PL
typedef intptr_t           Bits; // PL
typedef uint32_t           Bit32u; // PL
typedef int32_t            Bit32s; // PL
typedef uint16_t           Bit16u; // PL
typedef int16_t            Bit16s; // PL
typedef uint8_t            Bit8u; // PL
typedef int8_t             Bit8s; // PL

extern "C" void init_gus(void); // PL
extern "C" void get_samples(short *buf); // PL
extern "C" void write_gus(Bitu port,Bitu val, Bitu iolen); // PL
...
//		gus_chan->Enable(true);
		myGUS.basefreq = (Bit32u)((double)1000000/(1.619695497*(double)(myGUS.ActiveChannels)));
#if LOG_GUS
...
// PL static
void write_gus(Bitu port,Bitu val,Bitu iolen) {
...
void get_samples(short *buf) {
	...
	int buf32[2*len];
        Bitu i;
	//memset(buf32, 0, 2*len*4);
	for(i=0;i<2*len;i++) buf32[i] = 0;
        for(i=0;i<myGUS.ActiveChannels;i++)
                guschan[i]->generateSamples(buf32,len);
        for(i=0;i<len*2;i++) {
                Bit32s sample=((buf32[i] >> 13)*AutoAmp)>>9;
